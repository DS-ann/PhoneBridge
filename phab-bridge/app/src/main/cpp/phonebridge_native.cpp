...
struct hw_module_methods_t_min {
    int (*open)(const hw_module_t_min *module, const char *id, void **device);
};
...
Java_com_dsann_phonebridge_NativeAudioProbe_probeHal(JNIEnv *env, jclass) {
    ...
    /* Discovery only. Never call the vendor audio HAL's module->open()
     * from the application process. MTK HALs can depend on AudioFlinger,
     * AudioPolicy and vendor-global state and may abort or crash when opened
     * directly by an app. */
    ...
    if (!sym || symErr) {
        out << "HW_GET_MODULE_BY_CLASS:NOT_FOUND:" << (symErr ? symErr : "unknown") << "\n";
        dlclose(hardware);
        out << "NATIVE_AUDIO_HAL_PROBE:END";
        return env->NewStringUTF(out.str().c_str());
    }
    ...
    if (module) {
        ...
        out << "AUDIO_MODULE_OPEN_CALLBACK="
            << ((module->methods && module->methods->open) ? "AVAILABLE" : "UNAVAILABLE") << "\n";
    } else {
    ...
    out << "NATIVE_AUDIO_HAL_PROBE:END";
    dlclose(hardware);
    ALOGD("MTK audio HAL discovery probe completed safely");
    return env->NewStringUTF(out.str().c_str());
}
